...
import com.wao.common.registry.web.rest.dto.EurekaDTO;
...
public class EurekaResource {
	 ...
	 */
	@RequestMapping(value = "/eureka/applications", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	 ...
	 */
	@RequestMapping(value = "/eureka/lastn", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<Map<String, Map<Long, String>>> lastn() {
		...
		registry.getLastNCanceledInstances().stream().forEach(canceledInstance -> {
		...
		registry.getLastNRegisteredInstances().stream().forEach(registeredInstance -> {
	 ...
	 */
	@RequestMapping(value = "/eureka/replicas", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<List<String>> replicas() {
		List<String> replicas = new ArrayList<>();
		getServerContext().getPeerEurekaNodes().getPeerNodesView().stream().forEach(node -> {
			try {
				// The URL is parsed in order to remove login/password
				// information
	 ...
	 */
	@RequestMapping(value = "/eureka/status", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
